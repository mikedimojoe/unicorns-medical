...
export function PlayerDetail({ player, onBack, onUpdate }: Props) {
  ...
  return (
    <div className="p-6 space-y-6">
      <button onClick={onBack} className="flex items-center gap-2 text-sm transition-colors hover:text-white" style={{ color: 'var(--text2)' }}>
        <ArrowLeft size={16} /> Zurück
      </button>

      <div className="rounded-xl p-5 flex items-center gap-4"
        style={{ background: 'linear-gradient(135deg, var(--surface), var(--surface2))', border: '1px solid var(--border)' }}>
        <div className="w-16 h-16 rounded-full flex items-center justify-center text-2xl font-bold flex-shrink-0"
          style={{ background: 'linear-gradient(135deg, var(--team-primary), var(--accent))', color: '#fff' }}>
          {player.number}
        </div>
        <div className="flex-1">
          <h1 className="text-xl font-bold text-white">{player.name}</h1>
          <div className="flex items-center gap-3 mt-1">
            <span className="text-xs font-semibold px-2 py-0.5 rounded" style={{ background: 'var(--border)', color: 'var(--text2)' }}>{player.position}</span>
            <span className="text-xs" style={{ color: player.unit === 'Offense' ? 'var(--accent)' : 'var(--team-primary)' }}>{player.unit}</span>
            <span className={`text-xs px-2 py-0.5 rounded-full ${col.bg} ${col.text}`}>{player.status}</span>
          </div>
          {player.injury && <p className="text-sm mt-2" style={{ color: 'var(--text2)' }}>🤕 {player.injury}</p>}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <div className="rounded-xl p-5 space-y-4" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
          ...
          <button onClick={saveChanges}
            ...
            style={{ background: 'linear-gradient(135deg, var(--team-primary), var(--team-primary))' }}>
            Speichern
          </button>
        </div>
          ...
          <div className="rounded-xl p-5 space-y-4" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-semibold text-white flex items-center gap-2">
                <Brain size={16} style={{ color: 'var(--text2)' }} /> KI-Analyse
              </h2>
            ...
            <div>
              <h3 className="text-xs font-semibold mb-2" style={{ color: 'var(--text2)' }}>Behandlungsempfehlung</h3>
            ...
            <div>
              <h3 className="text-xs font-semibold mb-2" style={{ color: 'var(--text2)' }}>Return-to-Play Protokoll</h3>
        ...
        <div className="rounded-xl p-5" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
          ...
          <ResponsiveContainer width="100%" height={120}>
            <BarChart data={historyChartData} layout="vertical">
              ...
              <YAxis type="category" dataKey="name" width={80} tick={{ fontSize: 10, fill: 'var(--text2)' }} />
              <Tooltip
              ...
              <Bar dataKey="weeks" radius={4}>
                {historyChartData.map((_, i) => (
                  <Cell key={i} fill={i % 2 === 0 ? 'var(--team-primary)' : 'var(--accent)'} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
          ...
          <div className="mt-4 space-y-2">
            {player.injuryHistory.map(h => (
              <div key={h.id} className="rounded-lg px-3 py-2.5 flex items-start gap-3"
                ...
                <div className="text-xs min-w-20" style={{ color: 'var(--text3)' }}>
                ...
                <div className="flex-1">
                  ...
                  {h.notes && <div className="text-xs mt-0.5" style={{ color: 'var(--text2)' }}>{h.notes}</div>}
                </div>
                {h.etr && <div className="text-xs flex-shrink-0" style={{ color: 'var(--text2)' }}>ETR: {h.etr}</div>}
              </div>
            ))}
          </div>
        </div>
